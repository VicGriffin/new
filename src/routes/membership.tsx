  ...
  head: () => ({
    meta: [
      { title: "Affiliate Membership — AMTMTI" },
      {
        name: "description",
        content:
          "Join AMTMTI as a student, professional, institution, or corporate partner. Networking, research access, training discounts, and recognition.",
      },
    ...
    <PageShell>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {tiers.map((t) => (
            ...
            >
              ...
              {t.featured && (
                <span className="mt-3 text-[10px] font-bold uppercase tracking-wider text-emerald-brand">
                  Most popular
                </span>
              )}
              <h3 className={`mt-3 text-xl font-bold ${t.featured ? "text-white" : "text-navy"}`}>
                {t.t}
              </h3>
              <p className={`mt-1 text-sm ${t.featured ? "text-white/70" : "text-foreground/70"}`}>
                {t.desc}
              </p>
              <div
                className={`mt-4 text-2xl font-bold ${t.featured ? "text-emerald-brand" : "text-navy"}`}
              >
                {t.price}
              </div>
              <ul
                className={`mt-5 space-y-2 text-sm flex-1 ${t.featured ? "text-white/85" : "text-foreground/80"}`}
              >
                {t.features.map((f) => (
                  <li key={f} className="flex gap-2">
                    <Check className="size-4 text-emerald-brand shrink-0 mt-0.5" />
                    {f}
                  </li>
                ))}
              </ul>
      ...
      <section className="bg-soft border-y border-border">
        <div className="mx-auto max-w-3xl px-5 lg:px-8 py-20">
          <h2 className="text-3xl lg:text-4xl font-bold text-navy text-center">
            ...
            <div className="grid md:grid-cols-2 gap-5">
              ...
              <Field label="Profession" required>
                <input
                  ...
                  maxLength={120}
                  className={inp}
                  placeholder="e.g. Pharmacist, Nurse"
                  value={f.profession}
