  ...
  head: () => ({
    meta: [
      { title: "About — AMTMTI" },
      {
        name: "description",
        content:
          "Mission, vision, leadership, and strategic objectives of the Africa Medication Therapy Management Training Institute.",
      },
      ...
      <section className="mx-auto max-w-7xl px-5 lg:px-8 py-20 grid lg:grid-cols-2 gap-12">
        <div>
          ...
          <h2 className="mt-3 text-3xl lg:text-4xl font-bold text-navy">
            Closing a critical gap in African healthcare
          </h2>
          <p className="mt-5 text-foreground/75 leading-relaxed">
            Africa carries a growing burden of chronic disease, polypharmacy, and antimicrobial
            resistance. Yet structured medication therapy management training has historically been
            scarce. AMTMTI was created by African pharmacy and clinical leaders to change that —
            with curricula, research, and certification grounded in our health systems.
          </p>
        </div>
        <div>
          <span className="text-xs font-bold uppercase tracking-[0.18em] text-medical">
          ...
          <ul className="mt-6 space-y-3">
            {objectives.map((o) => (
              <li key={o} className="flex gap-3 text-foreground/85">
                <CheckCircle2 className="size-5 text-emerald-brand shrink-0 mt-0.5" />
                {o}
              </li>
            ))}
          </ul>
        ...
        <div className="mt-12 relative">
          ...
          <div className="space-y-10">
            {timeline.map((t, i) => (
              <div
                key={t.y}
                className={`relative lg:grid lg:grid-cols-2 lg:gap-12 ${i % 2 ? "lg:[&>*:first-child]:order-2" : ""}`}
              >
