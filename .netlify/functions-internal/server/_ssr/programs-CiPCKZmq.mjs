...
import { R as Route$f } from "./router-W4DiE8xe.mjs";
...
function Programs() {
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error: error2
      } = await supabase.from("programs").select("id,title,slug,summary,description,duration,level,certification,price_ksh,program_categories(name,slug)").eq("is_published", true).order("created_at");
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error: error2
      } = await supabase.from("program_categories").select("id,name,slug").order("name");
      if (error2) throw error2;
      return data ?? [];
    }
  });
  const filtered = reactExports.useMemo(() => {
    ...
    return (programs ?? []).filter((p) => {
      const matchCategory = activeCategory === "All" || p.program_categories?.slug === activeCategory;
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-wrap items-center justify-between gap-3", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-wrap gap-2", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: () => navigate({
            search: {}
          }), className: `rounded-full px-4 py-1.5 text-sm border transition ${activeCategory === "All" ? "bg-navy text-white border-navy" : "border-border text-foreground/75 hover:border-medical hover:text-medical"}`, children: "All" }),
          categories?.map((c) => /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: () => navigate({
            search: {
              category: c.slug
            }
          }), className: `rounded-full px-4 py-1.5 text-sm border transition ${activeCategory === c.slug ? "bg-navy text-white border-navy" : "border-border text-foreground/75 hover:border-medical hover:text-medical"}`, children: c.name }, c.id))
        ] })
      ] }),
        ...
        return /* @__PURE__ */ jsxRuntimeExports.jsxs("article", { className: "grid lg:grid-cols-12 gap-6 rounded-2xl border border-border bg-card overflow-hidden hover:shadow-xl hover:shadow-medical/5 transition", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "relative lg:col-span-3 bg-gradient-to-br from-medical to-emerald-brand p-8 text-white min-h-[180px]", children: [
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mt-auto pt-12", children: [
              ...
              categoryObj?.name && /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "mt-1 text-xs opacity-75", children: categoryObj.name })
            ] })
          ] }),
