...
import { g as getEffectiveRole, a as getUserStatus } from "./router-W4DiE8xe.mjs";
...
function AuthPage() {
  ...
  reactExports.useEffect(() => {
    async function checkSession() {
      ...
      const [role, status] = await Promise.all([getEffectiveRole(data.session.user.id), getUserStatus(data.session.user.id)]);
      if (role === "admin") {
        nav({
          to: "/admin",
          replace: true
