import { c as createServerRpc, a as createSupabaseAdminClient } from "./client.server-Ciid334F.mjs";
...
const getResourceDownloadUrl = createServerFn({
  ...
  if (!isAdmin && !resource.is_public) {
    ...
    } = await supabaseAdmin.from("course_enrollments").select("id").eq("user_id", context.userId).eq("program_id", resource.program_id).maybeSingle();
